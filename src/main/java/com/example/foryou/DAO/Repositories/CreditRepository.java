...
public interface CreditRepository extends JpaRepository<Credit,Integer> {
    ...
    @Query(value = "select c.* from credit c join user u on c.client_user_id=u.user_id  where ((c.refund_amount/c.nb_years)<=0.4*u.salaire) and c.credit_id=?1  ",nativeQuery = true)
    List<Credit> selectBySalary(int id);
